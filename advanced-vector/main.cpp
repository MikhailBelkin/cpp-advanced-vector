...
void Test6() {
    ...
    }
    {
        ...
        auto x = Obj::num_default_constructed + Obj::num_copied + Obj::num_moved + Obj::num_constructed_with_id
            + Obj::num_constructed_with_id_and_name - Obj::num_destroyed;
        assert(v.Size() == SIZE + 1);
        assert(v.Capacity() == SIZE * 2);
        assert(&*pos == &v[1]);
        assert(v[1].id == 1);
...
void TestInsertAdditional_move_noexcept_copy() {
    TestInsertAdditionalCopyImpl<C>(0u, SIZE);
    TestInsertAdditionalMoveImpl<C>(0u, SIZE);
}
void TestInsertAdditional_move_without_noexcept_copy() {
    TestInsertAdditionalCopyImpl<move_without_noexcept>(SIZE, 0u);
    TestInsertAdditionalMoveImpl<move_without_noexcept>(SIZE, 0u);
}
...
int main() {
    ...
    qwe.Emplace(&qwe[5], -1);
    qwe.Emplace(&qwe[0], -10);
    
    Vector <int>asd;

    asd.Emplace(asd.begin(), -20);

    for (int i = 0; i < qwe.Size(); i++) {
        std::cout << "qwe[" << i << "]=" << qwe[i] << std::endl;
